"""Runtime configuration for the EWA-Reward service."""
...
class Settings:
    """中文：EWA-Reward 服务运行时配置集合。
English: Runtime settings used by the EWA-Reward service."""
    api_base: str
    api_key: str
    model: str
    host: str
...
English: Build the service settings from environment variables and defaults."""
    ...
    return Settings(
        api_base=os.getenv("EWA_REWARD_API_BASE", "http://localhost:7000/v1"),
        api_key=os.getenv("EWA_REWARD_API_KEY", "dummy"),
        model=os.getenv("EWA_REWARD_MODEL", "/mnt/afs-h200/shared_model/Qwen3_6_27B/"),
        host=os.getenv("EWA_REWARD_HOST", "0.0.0.0"),
        port=_env_int("EWA_REWARD_PORT", 7024),
        log_root=Path(os.getenv("EWA_REWARD_LOG_ROOT", str(repo_root / "runs"))),
        save_inputs=_env_bool("EWA_REWARD_SAVE_INPUTS", True),
        llm_timeout=_env_float("EWA_REWARD_LLM_TIMEOUT", 600),
        max_retries=_env_int("EWA_REWARD_MAX_RETRIES", 10),
        motion_quality_path=os.getenv("EWA_REWARD_MOTION_QUALITY_PATH") or None,
        enable_motion_quality=_env_bool("EWA_REWARD_ENABLE_MOTION_QUALITY", False),
    )
